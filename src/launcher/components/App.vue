<template>
  <nav>
    <ul>
      <li class="row-container" v-for="site in siteListGet" v-bind:key="site.key">
        <a
          class="left"
          ...
          tabindex="0"
        >{{ site.url }}</a>
        <input class="btn" type="button" value="❌" v-on:click.prevent="onRemoveSite(site)">
      </li>
    </ul>

    <form id="addsiteForm" class="row-container" v-on:submit.prevent>
      <!-- <div width="100%"> -->
      <input class="left" v-model="siteUrl" type="url" placeholder="https://" autofocus>
      <!-- </div> -->
...
export default class App extends Vue {
  ...
  // @Action("removeSite") removeSiteAction: (site: Site) => void;
  removeSiteAction(site: Site): void {
    this.$store.dispatch("removeSite", site);
  }
  get siteListGet() {
    return (this.$store.state as SiteState).siteListReadonly;
  }
  onAddSite(): void {
    let siteUrl: string = "";
    try {
